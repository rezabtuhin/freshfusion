...
<x-user-layout title="Payment | Bkash" orders="{{ $count }}">
    <form id="pin-given" class="max-w-md mx-auto h-96 flex items-center justify-center bg-pink-700 p-4">
        ...
        <div>
            ...
            <button type="submit"
                    class="w-full mt-4 flex items-center justify-center gap-3 text-white bg-pink-900 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 hover:cursor-pointer">
                <span>Continue</span>
            ...
            <p class="text-white text-center">Taka <span
                    class="font-extrabold">{{ Session::get('don:amount'.Auth::user()->id) }}</span>
                will be deducted from your account</p>
        </div>
    </form>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Toast.fire({
                            ...
                            .then(() => {
                                setTimeout(function () {
                                    window.location.href = "/donation"; // Replace "your_url_here" with the URL you want to redirect to
                                }, 1);
                            });
                    },
